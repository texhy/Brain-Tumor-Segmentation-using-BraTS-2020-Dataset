...
class BraTSDataset(Dataset):
    ...
    def load_subject(self, subject_id: str) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Returns:
            ...
            mask: (D, H, W) array with segmentation labels
        """
        ...
        for modality in modalities:
            ...
            file_path_gz = os.path.join(subject_path, f"{subject_id}_{modality}.nii.gz")
            file_path_nii = os.path.join(subject_path, f"{subject_id}_{modality}.nii")
            
            if os.path.exists(file_path_gz):
                file_path = file_path_gz
            elif os.path.exists(file_path_nii):
                file_path = file_path_nii
            else:
                raise FileNotFoundError(f"Missing modality file for {modality}: {file_path_gz} or {file_path_nii}")
        ...
        mask_path_gz = os.path.join(subject_path, f"{subject_id}_seg.nii.gz")
        mask_path_nii = os.path.join(subject_path, f"{subject_id}_seg.nii")
        
        if os.path.exists(mask_path_gz):
            mask_path = mask_path_gz
        elif os.path.exists(mask_path_nii):
            mask_path = mask_path_nii
        else:
            raise FileNotFoundError(f"Missing segmentation file: {mask_path_gz} or {mask_path_nii}")
